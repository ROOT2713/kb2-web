...
r = requests.post(f"{BASE}/api/auth/login", json={"username": USER, "password": PASS}, timeout=10)
...
RULE_PASS = {
    ...
    "C09": ["湿度","%"],"C10": ["等保","测评","一年","1年"],
    "C11": ["密码","测评","一年","1年"],"C12": ["防静电地板","接地电阻","1"],
    ...
    "E05": ["22239","密码"],"E06": ["1717.2","测评"],
    "A09": ["2887","温度"],"A13": ["2887","湿度"],
    "H01": ["广州","政务信息化","管理办法"],"H02": ["验收测评","费用","取费"],
    "H03": ["软件造价","功能点"],"H04": ["36964","功能点"],
    "H06": ["南沙","信息化","管理"],"H07": ["东莞","造价","指南"],
    "H08": ["电子政务","建设","费用"],"H09": ["密码应用","方案","评估"],
    "H10": ["密评","安全","评估"],"Z01": ["等保","22239","要求"],
    "Z02": ["等保","28448","测评"],
}
NEEDS_REJECT = {
...
def evaluate(n, q_full, result):
    ...
    if any(w in ans[:500] for w in REJECT_TERMS): return "OVERREJECTION"
    if len(ans) < 100: return "ANSWER_TOO_SHORT"
    if n in NEEDS_REJECT:
        rej_terms = NEEDS_REJECT[n][1:]
        al = ans.lower()
        if any(t in al for t in rej_terms): return "PASS"  # correctly rejected
        if len(ans) > 200: return f"RULE_FAIL: should reject but answered ({ans[:60]}...)"
        return "UNCERTAIN"
    if n in RULE_PASS:
        missing = [kw for kw in RULE_PASS[n] if kw not in ans]
...
print(f"过拒: {rej_cnt} | 短答: {short_cnt} | 规则失败: {rule_fail} | 待LLM: {uncertain}")
