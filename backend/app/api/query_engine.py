...
async def _build_search_context(
    ...
    if _fee_q:
        ...
        # ── Fallback for v2 uploads (no parent_chunks): scan all_results for fee doc chunks ──
        if not _fee_chunks_to_inject and _all_fee_ids:
            _fee_id_set = set(_all_fee_ids)
            _v2_fee_chunks = []
            for _r in all_results:
                ...
                for _t in _r.get("tags", []):
                    if _t.startswith("doc_id:"):
                        _did = _t[7:]
                        break
                if _did and _did in _fee_id_set:
                    _v2_fee_chunks.append(_r)
            if _v2_fee_chunks:
                logger.info("[D2-B] Fallback: boosting %d v2 fee chunks from hindsight", len(_v2_fee_chunks))
                for _chunk in reversed(_v2_fee_chunks):
...
async def _generate_answer(
    ...
    if any(kw in _d2q_fee for kw in [
        ...
        _fee_rules = (
            ...
            "   h. **覆盖【输出要求】#2 — 费用类查询改用表格格式**：禁止使用`【问题类别】`+列表格式，必须输出标准 markdown 表格。表格格式为：第一行表头（费用类型 | 金额（万元）| 依据文件及条款），第二行分隔线（---|---|---），第三行起数据行。每行必须标注来源。多档位费率用多行展示。注意：你必须在回答中**实际生成**这个表格，不要输出指令或模板本身。\n"
            ...
            "      | 监理费 | 工程监理服务费/表2-27 | — |\n"
            "      如果提供的费率表是验收测评费但用户问的是等保，必须明确注明\"提供的费率表为验收测评费，与等保测评是不同费用类型，不应混用\"，然后在回答中提供等保测评服务费的定义和参考依据而非验收测评费率。\n"
        )
